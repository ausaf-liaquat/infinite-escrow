...
class FrontendController extends Controller
{
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        $valInputs = $request->except('_token', 'image_input','video_input', 'key', 'status', 'type', 'id');
        ...
        if ($type == 'data') {
            ...
            if ($request->hasFile('video_input')) {
                $inputContentValue['video'] = uploadFile($request->video_input,imagePath()['seo']['path'], null, @$content->data_values->video);
            }
        }else{
            if ($imgJson) {
            ...
            if ($videoJson) {
                ...
                foreach ($videoJson as $videoKey => $imgValue) {
                    $videoData = @$request->video_input;
                    //  dd($videoData,$request->video_input);
                    if (is_file($videoData)) { 
                       
                        try {
                            $inputContentValue[$videoKey] = $this->storeFile($videoJson,$type,$key,$videoData,$videoKey,@$content->data_values->$videoKey);
                        } catch (\Exception $exp) {
                            $notify[] = ['error', 'Could not upload the image.'];
                            return back()->withNotify($notify);
                        }
    ...
    protected function storeFile($imgJson,$type,$key,$image,$imgKey,$old_image = null)
    {
        $path = 'assets/images/frontend/' . $key;
        if ($type == 'element' || $type == 'content') {
            $size = @$imgJson
        ...
        return uploadFile($image, $path, null, $old_image);
    }
